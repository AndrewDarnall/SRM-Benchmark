 ...
 */
SRM::SRM(int _Q, int _max_regions, double _min_size, cv::Mat &image)
    : img(image) {
  ...
  this->delta =
      std::log(6.0) + 2.0 * std::log(static_cast<double>(this->imgSize));
...
void SRM::run() {
  ...
  std::vector<std::pair<int, int>> edgeList =
      this->makeEdgePairList(this->getImageRows(), this->getImageCols());
...
void SRM::sortEdgePairs(std::vector<std::pair<int, int>> &edgeList) {
  ...
  std::stable_sort(edgeList.begin(), edgeList.end(),
                   [&diffComparator](const std::pair<int, int> &a,
...
bool SRM::predicate(int parentA, int parentB) {
  ...
  cv::Vec3f comparrison = (this->img.at<cv::Vec3f>(0, parentA) -
                           this->img.at<cv::Vec3f>(0, parentB));
  comparrison = comparrison.mul(comparrison);

  auto checkPredicate = [](const cv::Vec3f &squaredDiff, double threshold) {
    return (squaredDiff[0] < threshold) && (squaredDiff[1] < threshold) &&
           (squaredDiff[2] < threshold);
  };
...
double SRM::evaluatePredicate(int parentA) {
  ...
  return ((std::pow(this->g, 2) /
           static_cast<double>(2 * this->Q * this->rank[parentA])) *
...
void SRM::merge(int parentA, int parentB) {
  ...
  cv::Vec3f color = ((this->img.at<cv::Vec3f>(0, parentA) * sum1) +
...
void SRM::mergeSmallerRegions() {
  ...
  if (maxParents.size() > this->maxRegions) {
    std::vector<int> allowedParents(maxParents.begin(),
                                    maxParents.begin() + this->maxRegions);
    for (int i = 0; i < this->getImageSize(); i++) {
      int r1 = this->getParent(i);
      int r2 = this->getParent(i - 1);
      if ((r1 != r2) && (std::find(allowedParents.begin(), allowedParents.end(),
                                   r1) == allowedParents.end())) {
        this->merge(r1, r2);
      }
    }
  }
...
std::vector<int> &SRM::retrieveMaxParents() {
  ...
  std::vector<std::pair<int, int>> countsVector(parentsCounts.begin(),
                                                parentsCounts.end());
...
int SRM::getImageRows() const { return this->imgRows; }

int SRM::getImageCols() const { return this->imgCols; }

int SRM::getImageSize() const { return this->imgSize; }
